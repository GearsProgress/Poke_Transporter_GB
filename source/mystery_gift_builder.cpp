...
void mystery_gift_script::build_script(PokeBox *box)
{
    ...
    static const byte track_2[] = {0xBC, 0x00, 0xBD, 0x38, 0xC4, 0x00, 0xBE, 0x60, 0xBF, 0x46, 0xC0, 0x40, 0x83, 0xD4, 0x51, 0x3C, 0x86, 0xD4, 0x8C, 0x53, 0x86, 0xD4, 0x8C, 0x54, 0x86, 0xD4, 0x92, 0xE8, 0x55, 0x99, 0x81, 0xB1};
    songLooker.add_track(track_2, sizeof(track_2));

    static const byte track_3[] = {0xBC, 0x00, 0xBD, 0x38, 0xC4, 0x00, 0xBF, 0x34, 0xBE, 0x6E, 0xC0, 0x40, 0xD4, 0x3C, 0x70, 0x86, 0xD4, 0x8C, 0x3B, 0x86, 0xD4, 0x8C, 0x3C, 0x86, 0xD4, 0x92, 0xEA, 0x3D, 0x9B, 0x83, 0xB1};
    songLooker.add_track(track_3, sizeof(track_3));

    static const byte track_4[] = {0xBC, 0x00, 0xBD, 0x38, 0xC4, 0x00, 0xBF, 0x34, 0xBE, 0x6E, 0xC0, 0x40, 0xD4, 0x39, 0x70, 0x86, 0xD4, 0x8C, 0x36, 0x86, 0xD4, 0x8C, 0x37, 0x86, 0xD4, 0x92, 0xEA, 0x39, 0x9B, 0x83, 0xB1};
    songLooker.add_track(track_4, sizeof(track_4));

    static const byte track_5[] = {0xBC, 0x00, 0xBD, 0x38, 0xC4, 0x00, 0xBF, 0x34, 0xBE, 0x6E, 0xC0, 0x40, 0xD4, 0x34, 0x70, 0x86, 0xD4, 0x8C, 0x32, 0x86, 0xD4, 0x8C, 0x34, 0x86, 0xD4, 0x92, 0xEA, 0x37, 0x9B, 0x83, 0xB1};
    ...
    static const byte track_instrument_1[] = {0xBC, 0x00, 0xBD, 0x00, 0xC4, 0x00, 0xBE, 0x7F, 0xBF, 0x40, 0xC0, 0x40, 0xD4, 0x21, 0x70, 0x86, 0xD4, 0x8C, 0x20, 0x86, 0xD4, 0x8C, 0x1F, 0x86, 0xDA, 0x8C, 0xD4, 0x2D, 0x86, 0x21, 0x8C, 0xD4, 0x86, 0xD4, 0x8C, 0xD4, 0x85, 0xB1};
    songLooker.add_track(track_instrument_1, sizeof(track_instrument_1));
    // ugly hack: patch the instrument
    songLooker.trackArrays[songLooker.trackArrays.size() - 1][3] = instrument;
...
#include "lookerRSE.h"
    ...
    {
        load_localized_charset(decompressed_store.text.gen3_charset, 3, ENGLISH);
        tte_erase_screen();
        int val = (curr_mg_index - MG_SCRIPT_SIZE) - four_align_value;
        ptgb_write_debug(decompressed_store.text.gen3_charset, "MG Script exceeded by ", true);
        ptgb_write_debug(decompressed_store.text.gen3_charset, ptgb::to_string(val), true);
        ptgb_write_debug(decompressed_store.text.gen3_charset, " bytes", true);
        while (true)
        {
        }
    }
    ...
    if (curr_section30_index > 4096) // Throw an error if the script is too large
    {
        ...
        ptgb_write_debug(decompressed_store.text.gen3_charset, "S30 Script exceeded by ", true);
        ptgb_write_debug(decompressed_store.text.gen3_charset, ptgb::to_string(val), true);
        ptgb_write_debug(decompressed_store.text.gen3_charset, " bytes", true);
        while (true)
        {

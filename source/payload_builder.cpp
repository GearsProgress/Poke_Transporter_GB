...
byte *generate_payload(GB_ROM curr_rom, int type, bool debug)
{
    ...
    else if ((curr_rom.generation == 1 && curr_rom.method == METHOD_MEW))
    {
        ...
        for (int i = 0; i < 11; i++)
        {
            z80_patchlist.add_byte(curr_rom.custom_name[i]);
        }
        ...
        for (int i = 0; i < distance; i++)
        {
            z80_patchlist.add_byte(i != 277 ? curr_rom.short_pkmn_name : 0xC8);
        }
        z80_patchlist.add_byte(curr_rom.pointer_pkmn_name);
        z80_patchlist.add_byte(0xFF);

        // Add in the code for reloading the script, keeping in mind the MEWs
        offset = 0x19E;

        // ld hl, hSerialConnectionStatus
        z80_patchlist.LD(HL, curr_rom.hSerialConnectionStatus | T_U16);
        z80_patchlist.LD(HL_PTR, (debug ? 0x02 : 0x01) | T_U8); // Make sure GB is the slave, master if debug
        offset += 4;                                            // Mew
        z80_patchlist.LD(HL, curr_rom.garbageDataLocation | T_U16);
        z80_patchlist.LD(DE, 0xC5D0 | T_U16);  // wSerialEnemyMonsPatchList
        z80_patchlist.JR((-47 & 0xFF) | T_I8); // Relative jump, since that's all we have space for

        offset -= 47;

        z80_patchlist.LD(BC, 0x00C2 | T_U16);
        z80_patchlist.CALL(curr_rom.Serial_ExchangeBytes | T_U16); // Refresh the patch section
        z80_patchlist.LD(A, (curr_rom.SaveSAVtoSRAM1 >> 16) | T_U8);
        z80_patchlist.CALL(curr_rom.Bankswitch | T_U16);
        z80_patchlist.CALL(0x79FC | T_U16); // Call LoadSAV1

        offset += 5; // Mew

        z80_patchlist.CALL(0x7A24 | T_U16);
        z80_patchlist.JP(0xC5D6 | T_U16);
        ...
        if (type == TRANSFER)
        {
            ...
            z80_patchlist.CALL(curr_rom.CableClub_TextBoxBorder | T_U16);
            z80_patchlist.LD(HL, curr_rom.transferStringLocation | T_U16);
            z80_patchlist.LD(DE, curr_rom.transferWaitString | T_U16);
            z80_patchlist.CALL(curr_rom.placeString | T_U16);
            ...
            z80_patchlist.LD(HL_PTR, (debug ? 0x02 : 0x01) | T_U8);         // Make sure GB is the slave, master if debug
            z80_patchlist.LD(HL, curr_rom.garbageDataLocation | T_U16);
            z80_patchlist.LD(DE, curr_rom.pkmnTransferArray | T_U16);
            z80_patchlist.LD(BC, 0x001E | T_U16); // Preamble does *not* count
            ...
            z80_patchlist.LD(HL, curr_rom.wRemoveMonFromBox | T_U16);
            z80_patchlist.LD(HL_PTR, 0x01 | T_U8); // != 0x00 specifies the current box
            z80_patchlist.LD(A, curr_rom.arrayCounter | T_U16);
            z80_patchlist.LD(E, A);
            z80_patchlist.LD(D, 0x00 | T_U8);
            z80_patchlist.LD(HL, curr_rom.pkmnTransferArray | T_U16);
            z80_patchlist.ADD(HL, DE);
            z80_patchlist.LD(E, 0xFF | T_U8);
            z80_patchlist.INC(A);
            z80_patchlist.LD(curr_rom.arrayCounter | T_U16, A);
            z80_patchlist.LD(A, curr_rom.wBoxCount | T_U16);
            z80_patchlist.LD(B, A);
            z80_patchlist.LD(A, HL_PTR);
            z80_patchlist.CP(A, E);
            z80_patchlist.JR(Z_F, 12 | T_I8);
            z80_patchlist.CP(A, B);
            z80_patchlist.JR(NC_F, (-27 & 0xFF) | T_I8);
            z80_patchlist.LD(HL, curr_rom.wWhichPokemon | T_U16);
            z80_patchlist.LD(HL_PTR, A);
            if (DONT_REMOVE_PKMN)
            {
...
int main() // Rename to "main" to send the payload to test_payload.txt
{
    ...
    if (true)
    {
        for (int i = 0; i < (sizeof(payload) / sizeof(byte)); i++)
        {
